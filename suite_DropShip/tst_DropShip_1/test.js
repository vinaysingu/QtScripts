...
function main()
{
  ...
  var appEdition = findApplicationEdition();
    ...
    //----- Enabling Tab View functionality ---------
    ...
    try
    {
      ...
      test.log("Purchase Order Setups made sucessfully:");
  }
  catch(e)
  {
  ...
  var sourceitem="TBOX1";
    ...
    //------- Verifying the Changes Made in the SO updated in PO and release the PO----
    try
    {
        waitForObjectItem(":xTuple ERP: *_QMenuBar", "Purchase");
        activateItem(":xTuple ERP: *_QMenuBar", "Purchase");
        ...
        waitForObject(":_list_XTreeWidget_3");
        if(object.exists("{column='0' container=':_list_XTreeWidget_3' text='"+ponumber1+"' type='QModelIndex'}"))
            test.pass("Purchase order linked to Sales Order is released:");
        else
            test.fail("Failed to release Purchase order linked to sales order:");
    ...
    try
    {
        ...
        if(!findObject(":List Open Purchase Orders.Unreleased_XCheckBox").checked)
            clickButton(":List Open Purchase Orders.Unreleased_XCheckBox");
        waitForObject(":Quotes.Query_QToolButton");
        clickButton(":Quotes.Query_QToolButton");  waitForObject(":Quotes.Query_QToolButton");
        clickButton(":Quotes.Query_QToolButton");
    ...
    try {
        ...
        if(OS.name == "Windows")
        {
            waitForObject(":Print Checks.Create EFT File_QPushButton");
            clickButton(":Print Checks.Create EFT File_QPushButton");
            snooze(0.5);
