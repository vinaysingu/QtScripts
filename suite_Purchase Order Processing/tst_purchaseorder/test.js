...
function main()
{
    ...
    if(object.exists(":Quotes.Close_QToolButton"))
    {
     ...
     activateItem(waitForObjectItem(":xTuple ERP: *_QMenuBar_3", "Window"));
     snooze(.5);
 ...
 // ------ Enabling Create EFT in Print Check screen ----
    if(appEdition == "PostBooks")
    {
    try{
    waitForObjectItem(":xTuple ERP: *_QMenuBar_3", "System");
    ...
    waitForObjectItem(":xTuple ERP: *.System_QMenu_2", "Setup...");
    activateItem(":xTuple ERP: *.System_QMenu_2", "Setup...");
    mouseClick(waitForObjectItem(":Setup._modules_QComboBox", "Accounting"), 39, 7, 0, Qt.LeftButton);
    clickTab(waitForObject(":Vendor.qt_tabwidget_tabbar_QTabBar"), "Accounts Payable");
    if(!findObject(":tab.Enable EFT Check Printing_QGroupBox").checked)
         mouseClick(waitForObject(":tab.Enable EFT Check Printing_QGroupBox"), 58, 6, 0, Qt.LeftButton);
    clickButton(waitForObject(":Setup.Save_QPushButton"));
   // clickButton(waitForObject(":_stack.No_QPushButton"));
}
catch(e)
{
    test.fail("Error in enabling Transmission tab"+e);
}
    }
