...
//        type(":Log In._port_QLineEdit",port);
        ...
        waitForObject(":_database_QLineEdit_2");
        type(":_database_QLineEdit_2", "<Tab>");
...
//----------costing----------------
function copyItem(sourceitem,targetitem,listprice,wsPrice)
 {
 try
{
  ...
  type(waitForObject(":Copy Item._listCost_XLineEdit"),wsPrice);
...
//-------- Pricing Schedule Assignment for Customer Ship To Pattern --------

function prcAssgShipToPatt(custname,prcAssg, pat)
{
    try
    {
        activateItem(waitForObjectItem(":xTuple ERP: *_QMenuBar", "Sales"));
        ...
        type(waitForObject(":Cash Receipt.VirtualClusterLineEdit_CLineEdit"), custname);
        nativeType("<Tab>");
        if(!findObject(":_customerGroup.Selected Cust. Ship-To Pattern:_QRadioButton").checked)
        {
            clickButton(waitForObject(":_customerGroup.Selected Cust. Ship-To Pattern:_QRadioButton"));
            
        }
        snooze(0.5);
        waitForObject(":_customerGroup._shiptoPattern_XLineEdit");
        type(":_customerGroup._shiptoPattern_XLineEdit",".*");
        nativeType("<Tab>");
        snooze(0.5);
        waitForObject(":_ipshead_XComboBox");
        clickItem(":_ipshead_XComboBox",prcAssg,0, 0, 5, Qt.LeftButton);
        ...
        if(object.exists("{column='2' container=':xTuple ERP: *._ipsass_XTreeWidget' text='"+custname+"' type='QModelIndex'}") && object.exists("{column='0' container=':xTuple ERP: *._ipsass_XTreeWidget' text='"+pat+"' type='QModelIndex'}"))
            test.pass("Pricing Schedule Assigment by Customer Ship-to Pattern created successfully");
        else  
            test.fail("Error in Assigning Pricing schedule to Customer Shipt To Pattern"); 
        clickButton(waitForObject(":Select Order for Billing.Close_QPushButton"));
    }
...
function prcAssgCustTypPatt(custname,prcAssg, pat, mPSName)
{
    try
    {
        activateItem(waitForObjectItem(":xTuple ERP: *_QMenuBar", "Sales"));
        ...
        clickButton(waitForObject(":_lineItemsPage.New_QPushButton_2"));
        snooze(1);
        if(!findObject(":_customerGroup.Customer Type Pattern:_QRadioButton").checked)
        {
            clickButton(waitForObject(":_customerGroup.Customer Type Pattern:_QRadioButton"));
            
        }

        snooze(0.5);
        waitForObject(":_customerGroup._customerType_XLineEdit");
        type(":_customerGroup._customerType_XLineEdit",".*");
        nativeType("<Tab>");
        snooze(0.5);
        waitForObject(":_ipshead_XComboBox");
        clickItem(":_ipshead_XComboBox",prcAssg,0, 0, 5, Qt.LeftButton);
        ...
        if(object.exists("{column='3' container=':xTuple ERP: *._ipsass_XTreeWidget' text='"+pat+"' type='QModelIndex'}") && object.exists("{column='4' container=':xTuple ERP: *._ipsass_XTreeWidget' text='"+mPSName+"' type='QModelIndex'}"))
            test.pass("Pricing Schedule Assigment by Customer Ship-to Pattern created successfully");
        else  
            test.fail("Error in Assigning Pricing schedule to Customer Type Pattern"); 
        clickButton(waitForObject(":Select Order for Billing.Close_QPushButton"));
    }
